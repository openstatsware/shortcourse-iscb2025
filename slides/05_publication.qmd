...
_The goal of this chapter is to prepare for the exercise of adding a licence and publish your package's website. One tool is to use `pkgdown`_
...
- After installing `pkgdown`, run `usethis::use_pkgdown()` to get started
  - nested in this [call](https://pkgdown.r-lib.org/articles/pkgdown.html) includes : `use_pkgdown()`, `use_github_pages()`, `use_github_action("pkgdown")`
...
- Easy setup from your R console: `usethis::use_github_action_check_standard()`
  - _runs a standard check `R CMD` on three major operating systems._
...
- Git can tag specific points in the code history as being important, like "bookmarks"
...
```{Bash}
#| eval: false
git diff v1.0 v1.1 # minor update
```
...
```{r}
#| eval: false
compareVersion("3.14", "3.15") # minor update
```
