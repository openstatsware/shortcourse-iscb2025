...
This section is adapted from slides by Michael Mayer and Lukas A. Widmer, which they prepared and released under the CC BY 4.0 license, see their course [Go fastR – how to make R code fast(er) and run it on high performance compute (HPC) clusters](https://luwidmer.github.io/fastR-website/materials.html).
...
- In a second version of the function, `impl_2`, only update vectors inside the loop and then create a `tibble` once at the end. How much time does it save?
- In a third version `impl_3` only subset the column instead of the whole `data.frame`. How much faster does it get?
- Homework: Try to come up with a fourth version `impl_4` that uses `Rcpp`. Was it worth the effort?
  
# License Information

- Creators (initial authors): 
  Michael Mayer and Lukas A. Widmer, see their course [Go fastR – how to make R code fast(er) and run it on high performance compute (HPC) clusters](https://luwidmer.github.io/fastR-website/materials.html)
